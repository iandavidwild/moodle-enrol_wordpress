...
/**
 * WordPress enrolment plugin implementation.
 * @author Ian Wild
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class enrol_wordpress_plugin extends enrol_plugin {
    ...
    public function get_info_icons(array $instances) {
        ...
        foreach ($instances as $instance) {
            if ($this->can_wordpress_enrol($instance, false) !== true) {
                ...
                // would hide wordpress-enrolment icons from guests.
                continue;
            }
        ...
        $icons = array();
        if ($nokey) {
            $icons[] = new pix_icon('withoutkey', get_string('pluginname', 'enrol_wordpress'), 'enrol_wordpress');
        }
        if ($key) {
            $icons[] = new pix_icon('withkey', get_string('pluginname', 'enrol_wordpress'), 'enrol_wordpress');
        }
        return $icons;
    }
    ...
    public function show_enrolme_link(stdClass $instance) {
        ...
        if (true !== $this->can_wordpress_enrol($instance, false)) {
            return false;
        }
    ...
    public function can_add_instance($courseid) {
        ...
        if (!has_capability('moodle/course:enrolconfig', $context) or !has_capability('enrol/wordpress:config', $context)) {
            return false;
        }
    ...
    public function enrol_page_hook(stdClass $instance) {
        ...
        require_once("$CFG->dirroot/enrol/wordpress/locallib.php");
    ...
    /**
     * Checks if user can WordPress enrol.
     *
     * @param stdClass $instance enrolment instance
     ...
     */
    public function can_wordpress_enrol(stdClass $instance, $checkuserenrolment = true) {
        ...
        if ($instance->enrolstartdate != 0 and $instance->enrolstartdate > time()) {
            return get_string('canntenrolearly', 'enrol_wordpress', userdate($instance->enrolstartdate));
        }

        if ($instance->enrolenddate != 0 and $instance->enrolenddate < time()) {
            return get_string('canntenrollate', 'enrol_wordpress', userdate($instance->enrolenddate));
        }
        ...
        if ($instance->customint3 > 0) {
            ...
            if ($count >= $instance->customint3) {
                ...
                return get_string('maxenrolledreached', 'enrol_wordpress');
            }
        }
    ...
    public function get_enrol_info(stdClass $instance) {
        ...
        $instanceinfo->status = $this->can_wordpress_enrol($instance);

        if ($instance->password) {
            $instanceinfo->requiredparam = new stdClass();
            $instanceinfo->requiredparam->enrolpassword = get_string('password', 'enrol_wordpress');
        }
        ...
        if ((true === $instanceinfo->status) && $instance->password) {
            $instanceinfo->wsfunction = 'enrol_wordpress_get_instance_info';
        }
        return $instanceinfo;
    }
    ...
    protected function email_welcome_message($instance, $user) {
        ...
        if (trim($instance->customtext1) !== '') {
            ...
            $messagetext = get_string('welcometocoursetext', 'enrol_wordpress', $a);
            $messagehtml = text_to_html($messagetext, null, false, true);
        }

        $subject = get_string('welcometocourse', 'enrol_wordpress', format_string($course->fullname, true, array('context'=>$context)));
    ...
    public function sync(progress_trace $trace, $courseid = null) {
        ...
        $trace->output('Verifying wordpress-enrolments...');
        ...
        $sql = "SELECT e.*, ue.userid
                  ...
                  JOIN {enrol} e ON (e.id = ue.enrolid AND e.enrol = 'wordpress' AND e.customint2 > 0)
        ...
        $trace->output('...user wordpress-enrolment updates finished.');
        $trace->finished();
    ...
    /**
     * Returns the user who is responsible for wordpress enrolments in given instance.
     *
     * Usually it is the first editing teacher - the person with "highest authority"
     * as defined by sort_by_roleassignment_authority() having 'enrol/wordpress:manage'
     * capability.
     *
    ...
    protected function get_enroller($instanceid) {
        ...
        if ($users = get_enrolled_users($context, 'enrol/wordpress:manage')) {
    ...
    public function get_user_enrolment_actions(course_enrolment_manager $manager, $ue) {
        ...
        $params['ue'] = $ue->id;
        if ($this->allow_unenrol($instance) && has_capability("enrol/wordpress:unenrol", $context)) {
            $url = new moodle_url('/enrol/unenroluser.php', $params);
            $actions[] = new user_enrolment_action(new pix_icon('t/delete', ''), get_string('unenrol', 'enrol'), $url, array('class'=>'unenrollink', 'rel'=>$ue->id));
        }
        if ($this->allow_manage($instance) && has_capability("enrol/wordpress:manage", $context)) {
    ...
    public function can_delete_instance($instance) {
        $context = context_course::instance($instance->courseid);
        return has_capability('enrol/wordpress:config', $context);
    }
    ...
    public function can_hide_show_instance($instance) {
        ...
        if (!has_capability('enrol/wordpress:config', $context)) {
            return false;
        }
    ...
    public function edit_instance_form($instance, MoodleQuickForm $mform, $context) {
        ...
        $mform->addElement('passwordunmask', 'password', get_string('password', 'enrol_wordpress'), $passattribs);
        $mform->addHelpButton('password', 'password', 'enrol_wordpress');
        ...
        $mform->addElement('select', 'customint1', get_string('groupkey', 'enrol_wordpress'), $options);
        $mform->addHelpButton('customint1', 'groupkey', 'enrol_wordpress');

        $roles = $this->extend_assignable_roles($context, $instance->roleid);
        $mform->addElement('select', 'roleid', get_string('role', 'enrol_wordpress'), $roles);

        $options = array('optional' => true, 'defaultunit' => 86400);
        $mform->addElement('duration', 'enrolperiod', get_string('enrolperiod', 'enrol_wordpress'), $options);
        $mform->addHelpButton('enrolperiod', 'enrolperiod', 'enrol_wordpress');
        ...
        $mform->addElement('date_time_selector', 'enrolstartdate', get_string('enrolstartdate', 'enrol_wordpress'), $options);
        $mform->setDefault('enrolstartdate', 0);
        $mform->addHelpButton('enrolstartdate', 'enrolstartdate', 'enrol_wordpress');

        $options = array('optional' => true);
        $mform->addElement('date_time_selector', 'enrolenddate', get_string('enrolenddate', 'enrol_wordpress'), $options);
        $mform->setDefault('enrolenddate', 0);
        $mform->addHelpButton('enrolenddate', 'enrolenddate', 'enrol_wordpress');

        $options = $this->get_longtimenosee_options();
        $mform->addElement('select', 'customint2', get_string('longtimenosee', 'enrol_wordpress'), $options);
        $mform->addHelpButton('customint2', 'longtimenosee', 'enrol_wordpress');

        $mform->addElement('text', 'customint3', get_string('maxenrolled', 'enrol_wordpress'));
        $mform->addHelpButton('customint3', 'maxenrolled', 'enrol_wordpress');
        ...
        if (count($cohorts) > 1) {
            $mform->addElement('select', 'customint5', get_string('cohortonly', 'enrol_wordpress'), $cohorts);
            $mform->addHelpButton('customint5', 'cohortonly', 'enrol_wordpress');
        } else {
        ...
        $mform->addElement('advcheckbox', 'customint4', get_string('sendcoursewelcomemessage', 'enrol_wordpress'));
        $mform->addHelpButton('customint4', 'sendcoursewelcomemessage', 'enrol_wordpress');

        $options = array('cols' => '60', 'rows' => '8');
        $mform->addElement('textarea', 'customtext1', get_string('customwelcomemessage', 'enrol_wordpress'), $options);
        $mform->addHelpButton('customtext1', 'customwelcomemessage', 'enrol_wordpress');
    ...
    public function edit_instance_validation($data, $files, $instance, $context) {
        ...
        if ($data['status'] == ENROL_INSTANCE_ENABLED) {
            if (!empty($data['enrolenddate']) and $data['enrolenddate'] < $data['enrolstartdate']) {
                $errors['enrolenddate'] = get_string('enrolenddaterror', 'enrol_wordpress');
            }
        }
    ...
    /**
     * Gets a list of roles that this user can assign for the course as the default for wordpress-enrolment.
     *
     * @param context $context the context.
     * @param integer $defaultrole the id of the role that is set as the default for wordpress-enrolment
